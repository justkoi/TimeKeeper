...
class CEMRadioButton : public CEMUIEvent
{
private:
	enum RBUTTONSTATE {RELEASE = 0, HIGHLIGHT, CHECK, DOWN};
	enum RBUTTONSOUND {CLICK = 0, OVER};
...
private:
	...
	stEMVec3				m_vParent;			//!< 부모좌표
	stEMVec3				m_vRender;			//!< 실질적으로 그려지는 좌표
	
	RBUTTONSTATE			m_eButtonState;		//!< 버튼의 상태
...
public:
	CEMRadioButton(UINT id, UINT group, const char* cpParent, const char* cpDefault, const char* cpOver, const char* cpCheck, stEMVec3& vPos, stEMVec3& vParent, UIEvent* pUIEvent, bool bCheck = false);
	~CEMRadioButton();
};