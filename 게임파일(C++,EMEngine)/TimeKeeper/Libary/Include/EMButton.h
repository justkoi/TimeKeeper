...
class CEMButton : public CEMUIEvent
{
private:
	enum BUTTONSTATE {RELEASE = 0, HIGHLIGHT, PUSH};
	enum BUTTONSOUND {CLICK = 0, OVER};

private:
	std::vector<CEMPlane*>	m_Plane;				//!< 버튼의 이미지들을 담아 놓는 vector
	//std::vector<CEMAnimation*> m_Animate;			//!< 버튼의 애니메이션을 담아 놓은 vector
	...
	BUTTONSTATE				m_eButtonState;			//!< 버튼의 상태
...
public:
	void SetMirror(eMirrorType eType);
	/**
...
public:
	CEMButton(UINT id, const char* cpParent, const char* cpDefault, const char* cpOver, const char* cpClick, stEMVec3& vPos, stEMVec3& vParent, UIEvent* pUIEvent);
//	CEMButton(UINT id, const char* cpParent, CEMAnimation* pDefault, CEMAnimation* pOver, CEMAnimation* pClick, stEMVec3& vPos, stEMVec3& vParent, UIEvent* pUIEvent);
	~CEMButton();
};